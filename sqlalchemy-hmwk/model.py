...
class Employee(db.Model):
    ...
    def __repr__(self):
        return "<Employee id={emp_id} name={name}>".format(emp_id=self.emp_id,
                                                           name=self.name)
...
class Department(db.Model):
    ...
    def __repr__(self):
        return "<Dept id={dept_code} name={dept}>".format(dept_code=self.dept_code,
                                                          dept=self.dept)
...
def connect_to_db(app):
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql:///hw_emp'
